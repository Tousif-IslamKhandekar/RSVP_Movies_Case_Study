...
-- columns that have null values are country, worlwide_gross_income, languages, production_company.
...
-- We can observe that 2017 has the highest number of movies released.
...
/* We can observe a trend here where most number of movies were released from Sept-Oct then Jan-March periods
with highest number of movies being released in March.*/
...
FROM
	movies_2019_US_India;
    
    
-- There are 1059 movies that were released in the year 2019 in India and US alone.
...
-- Drama genre has highest number of movies produced with 4285.
...
-- To be exact there are exactly 3289 movies that belong to one genre.
...
-- Thriller is in the third position when it comes to number of movies produced.
...
WITH movie_critical_ranking AS
(
SELECT 
	m.title,
...
/* I have used RANK() and DENSE_RANK() both to compare the output. ROW_NUMBER() can also be handy here where we can filter out 
the hard top 10 movies.
In anywhich case Fan will be among the top 10.*/
...
-- ROW_NUMBER() function can also come handy to hard filter out just the top 10.


/* Dream Warrior Pictures or National Theatre Live production houses have produced most number of hit movies.
These will be the dream production houses for anyone trying to make a movie./*
...
-- We can see here, Drama genre has most number of popular movies produced in the US during March of 2017.
...
SELECT
    median_rating,
    COUNT(m.id) as number_of_movies_released
FROM
	movie m
    INNER JOIN 
    ratings r
...
/* Approx 361 movies were released having a median rating 8 between the given period.*/
...
-- German movies have got more votes than Italian movies.
...
-- We can see that name column does not have any null values.
...
FROM 
    ...
    top3_genres tg 
    ON 
    g.genre=tg.top_genres
WHERE 
	r.avg_rating>8
GROUP BY 
	n.name
ORDER BY 
	count(dm.movie_id) DESC LIMIT 3;
...
SELECT 
    ...
    RANK() OVER (ORDER BY r.avg_rating DESC,sum(r.total_votes) DESC) as actor_rank
FROM
	names as n 
    INNER JOIN 
    role_mapping as rm 
...
-- Top Indian Actor is Vijay Sethupathi with an average rating of 8.7.
